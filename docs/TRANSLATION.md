# 🌍 GamersCommunity Front — Angular i18n (JSON Format)
...
<label i18n="@@users.login.email">Email address</label>
<input type="email" />

<label i18n="@@users.login.password">Password</label>
<input type="password" />

<button i18n="@@users.login.submit">Sign in</button>
```

### ✅ In TypeScript Files

You can translate text dynamically in your code using Angular’s `$localize` function:
...
```json
{
    "users.login.title": "Log in to your account",
    "users.login.email": "Email address",
    "users.login.password": "Password",
    "users.login.submit": "Sign in"
}
```
...
```json
{
    "users.login.title": "Connexion à votre compte",
    "users.login.email": "Adresse e-mail",
    "users.login.password": "Mot de passe",
    "users.login.submit": "Se connecter"
}
```
...
| ------------------- | ----------------------- | ------------------------------ |
| Page / Feature      | `feature.section.key`   | `users.login.title`            |
...
- Keep JSON files **organized by domain** (users, navbar, errors, etc.).
...
## 🧩 7. Configuration Reference (angular.json)
